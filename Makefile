...
format:
	...
	@-$(FORMATTER) -style=file -i $(INCLUDE_DIR)/*
	@-$(FORMATTER) -style=file -i $(TESTS_DIR)/*
...
setup:
	...
	@sudo pacman -S --noconfirm --needed bash libarchive lsb-release wget gnupg

	# Install LLVM tools required for building the project
	@sudo pacman -S --noconfirm --needed afl++ clang lld lldb lldb-mi llvm llvm-libs libc++ libclc polly openmp libunwind wasmer

	# Install Clang development tools
	@sudo pacman -S --noconfirm --needed compiler-rt valgrind bear

	# Install CUnit testing framework
	@sudo pacman -S --noconfirm --needed cunit
